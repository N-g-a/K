...
function addTableProducts() {
    ...
    for (var i = 1; i < list_products2.length; i++) {
...
function themSanPham() {
    ...
    setListProducts(list_products2);
...
function xoaSanPham(masp, tensp) {
    if (window.confirm('Bạn có chắc muốn xóa ' + tensp)) {
        // Xóa
        for (var i = 0; i < list_products2.length; i++) {
            if (list_products1[i].masp == masp) {
                list_products1.splice(i, 1);
            }
        }
...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["Apple", "Samsung", "Oppo", "Nokia", "Xiaomi", "Vivo"];
    ...
    for (var i = 0; i < u.length; i++) {
        for (var j = 0; j < u[i].donhang.length; j++) {
            ...
            for (var s of u[i].donhang[j].sp) {
                var timsp = timKiemTheoMa(list_products1, s.ma);
            ...
            var sps = '';
            for (var s of u[i].donhang[j].sp) {
                sps += `<p style="text-align: right">` + (timKiemTheoMa(list_products1, s.ma).name + ' [' + s.soluong + ']') + `</p>`;
            }
            ...
            for (var s of u[i].donhang[j].sp) {
                danhSachSanPham.push({
                    sanPham: timKiemTheoMa(list_products1, s.ma),
                    soLuong: s.soluong,
                });
            }
