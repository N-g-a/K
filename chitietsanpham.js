...
window.onload = function() {
    ...
    var tags = ["Samsung", "iPhone", "Xiaomi", "Oppo", "Vivo", "Nokia"];
    for (var t of tags) addTags(t, "index.html?search=" + t, true);

    phanTich_URL_chiTietSanPham();

    // autocomplete cho khung tim kiem
    autocomplete(document.getElementById('search-box'), list_products1);
...
function phanTich_URL_chiTietSanPham() {
    ...
    sanPhamHienTai = timKiemTheoMa(list_products2, maProduct);
    if (!sanPhamHienTai) return khongTimThaySanPham();

    var divChiTiet = document.getElementsByClassName('chitietSanpham')[0];

    // Đổi title
    document.title = nameProduct + ' Hoài Linh Smartphone - iPhone ';
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        case 'tragop':
            ...
            return `Khách hàng có thể mua trả góp sản phẩm với ` + span + ` thời hạn 6 tháng kể từ khi mua hàng.`;
        ...
        case 'moiramat':
            return `* Khách hàng sẽ được test máy miễn phí tại cửa hàng. <br>
                    * Có thể đổi trả lỗi trong vòng 30 ngày.`;

        case 'giareonline':
            var del = stringToNum(sp.price) - stringToNum(sp.promo.value);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Sản phẩm sẽ được giảm ` + span + `₫ khi mua hàng online bằng thẻ Viettinbank `;
...
function suggestion() {
    ...
    const sanPhamTuongTu = list_products2
        // Lọc sản phẩm trùng
    ...
    if (sanPhamTuongTu.length) {
        ...
        addKhungSanPham(sanPhamTuongTu, 'Gợi ý một số sản phẩm', ['#434aa8', '#ec1f1f'], div);
    }
}